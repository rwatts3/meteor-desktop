...
question('Do you want to use another path (yes/no)? ')
    ...
    .then(function(githubUsername) {
        ...
        exec('meteor create meteor-desktop-test-app');
    ...
    .then(function() {
        console.log('Linking...');
        return spawn(npm, ['link'], path.join(resolvedPath, 'meteor-desktop-test-suite'));
    })
    .then(function() {
    ...
    .then(function() {
        return spawn(npm, ['run', 'desktop'], path.join(resolvedPath, 'meteor-desktop-test-app'));
    })
    .then(function() {
        return spawn(npm, ['run', 'test-desktop', '--', '-b'], path.join(resolvedPath, 'meteor-desktop-test-app'));
    })
    .then(function() {
      finish();
    })
